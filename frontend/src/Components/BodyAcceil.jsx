    ...
    <div id="controls-carousel" className="relative w-full " data-carousel="static">
      <div className="relative h-56 overflow-hidden md:h-[600px]">
        ...
        <div class="absolute top-40 right-0 p-7 w-[470px] h-[290px] bg-[#004A93] text-white p-6 overflow-hidden">
            ...
            <p class="text-[16px] line-clamp-6 w-[360px]">{image.textImage}</p>
        </div> 
    ...
    return (
        ...
        <div className="flex items-center justify-center gap-2 w-full my-6">
            ...
            <div className="flex items-center gap-2 px-4">
                ...
                <span className="text-2xl font-semibold text-[#004a93]">{titre.toUpperCase()}</span>
            </div>
            <div className="flex-grow h-[1.5px] bg-[#004a93]"></div>
        </div>
        </>
...
function CardActualitess({image = imageCardAcctualitess, GroupeCible, Titre, date}) {
  return (
    <>
         <div className="w-[470px] h-[460px] bg-[#F2F2F2] rounded-2xl">
          <div className="absolute w-[330px] h-[300px] bg-[#004a93] clip-trapezoid rounded-t-2xl p-5">
            <div className="w-[130px] h-9 bg-[#00904a] p-2 mt-10 font text-white font-bold mb-5">{GroupeCible}</div>
            <div className="flex flex-col gap-6 p-3 max-w-[200px]">
              <p className="font text-[16px] font-bold text-white ">{Titre.toUpperCase()}</p>
              <p className="font text-[14px] text-white ">ISTA MOHAMED EL FASSI ERRACHIDIA</p>
            </div>
          </div>
          <img src={image} alt="" className='w-full h-[300px] ms-0 rounded-t-2xl'/>
        <div className="h-9 w-f bg-[#A4A3A3] p-2">
          <span className='flex justify-center gap-2'>
            <img src="https://img.icons8.com/?size=100&id=kCYgJWe7oksb&format=png&color=FFFFFF" alt="" className='size-6'/>
            <span className='text-white'>{date}</span>
          </span>
        </div>
          <div className='p-7 font font-bold text-[20px] text-center'>{Titre.toUpperCase()}</div>
      </div>
    </>
  )
}
    ...
    <>
      <div className="w-full mt-5 mb-5  h-[600px] py-4 container mx-auto">
        <div className="flex justify-start gap-[1px] p-10">
          <div id='images' className="h-[500px] w-[700px] flex justify-start gap-5 p-5">
            <div className="">
              <img src={imageCardAcctualitess} alt="" className='w-[430px] h-[450px] rounded-xl'/>
            </div>
            <div className="flex flex-col justify-start gap-5">
              <img src={image2} alt="" className='w-[350px] h-[215px] rounded-xl'/>
              <img src={image3} alt="" className='w-[350px] h-[215px] rounded-xl'/>
            </div>
          </div>
          <div id='Content' className=" h-[500px] w-[600px] p-5">
            <h1 className="text-[#004A93] font-bold font text-[30px] ">À LA DECOUVERTE D’ISTA MOHAMED EL FASSI</h1>
            <div className="font text-[30px] font-medium py-5 ">{"Notre institut, notre engagement.".toUpperCase()}</div>
            <p className="font text-[#707070] text-[20px] max-w-[600px] py-5">
            L’Institut Spécialisé de Technologie Appliquée Mohamed El Fassi d’Errachidia est un établissement public relevant de l’OFPPT.
            Il a pour mission de former des techniciens et des professionnels qualifiés,
            capables de répondre aux besoins du marché du travail et de contribuer au développement économique et social, au niveau régional et national.
            </p>
            <div className="flex gap-4 py-7">
              <span className='font-bold text-[23px]'>LA SUITE</span>
              <svg width="13" height="32" viewBox="0 0 12 22" fill="none"  xmlns="http://www.w3.org/2000/svg">
                <path d="M0.0950232 2.2689L1.76711 0.598391L10.8745 9.70265C11.0213 9.84853 11.1378 10.022 11.2173 10.2131C11.2968 10.4042 11.3378 10.6091 11.3378 10.8161C11.3378 11.023 11.2968 11.2279 11.2173 11.419C11.1378 11.6101 11.0213 11.7836 10.8745 11.9295L1.76711 21.0385L0.0965986 19.3679L8.64455 10.8184L0.0950232 2.2689Z" fill="#2B2B2B"/>
              </svg>
            </div>
          </div>
        </div>
  ...
  return (
    <>
      <div className="flex flex-col items-center gap-2 h-[250px] w-auto ">
        ...
        <span className='text-[#004A93] font-extrabold text-[30px] '>{Number}</span>
        <span className=' font-bold text-[25px]'>{GroupeCible.toUpperCase()}</span>
      </div>
    </>
  )
}
...
function Partenaire() {
  return (
    <div className='w-full container mx-auto'>
      <CreateLine 
        icone={"https://img.icons8.com/?size=100&id=xGUZ15gQQ60G&format=png&color=004A93"} 
